...
export function createLab(canvas, onReady) {
  ...
  function render(time, dt, state) {
    ...
    const excitement = state.boost;
    fly.position.y = 1.43 + Math.sin(move * 2.3) * .014 + Math.sin(move * 39) * excitement * .017;
    head.rotation.z = Math.sin(move * .7) * .025 + excitement * .07;
    wings.forEach((w, i) => { w.rotation.x = (i ? 1 : -1) * (Math.sin(move * (excitement > .1 ? 49 : 13)) * (.025 + excitement * .21) + Math.sin(move * .61) * .012); });
